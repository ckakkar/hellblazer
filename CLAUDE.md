...
- **Volume:** `sum(weight_kg * reps)` for working sets — aggregate per session, per exercise, and rolled up per `primary_muscle` (secondary muscles count at 0.5 weight toward their muscle's volume — implement this in the aggregation layer, documented).
