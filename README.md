...
- **Build numbers** count up by themselves (`run number.attempt`). **To ship a new version number**, bump `MARKETING_VERSION` on every target in `ios/App/App.xcodeproj` (the app, the widgets and the watch must match). The app is on **1.4**.
