...
- **Apple Watch app** (`ios/App/Watch`, watchOS 10+, so the first Apple Watch SE), laid out like Apple's Workout app. Start from a list of workout tiles (next programmed day first) with a 3-2-1 countdown; then swipe between Controls (End, Pause/Resume, Undo Set, Exercises), the logger, Metrics (a yellow running clock with hundredths, active calories, heart rate, sets, volume) and Now Playing. Log sets with the Digital Crown (weight and reps copy forward, with last session's set as the target), rest with a ring countdown that taps your wrist, and finish on a Summary screen. **Complications** put it on the watch face and in the Smart Stack: this week against your plan, your next workout, or the workout you're in with its clock (`ios/App/WatchWidgets`, reading what the watch app shares through the team's keychain group). It records the workout as a HealthKit workout session with live heart rate and calories, which also keeps it on screen when you raise your wrist; the phone then skips its own Health copy. It talks to the server directly (`/api/watch`, with a token the phone hands over through WatchConnectivity), so the phone can stay in your bag; sets logged offline wait on the watch. Starting a workout on the phone opens it on the watch, sets logged on the watch appear in the phone's logger when you look, and a finish on either ends it on both.
...
- **Feels like an app, not a web page.**
  ...
  - Swipe a workout left in History to delete it, as in Mail, with a few seconds to Undo (`SwipeRow`).
  - Search fields behave like iOS's: a Search key that puts the keyboard away, no autocorrect, a round clear button (`SearchField`).
...
- **The native code:** `ios/App/App` (the app's plugin `HellBlazerNativePlugin`, the view controller and router, App Intents, `WatchBridge`, Spotlight), `ios/App/Widgets` (the widgets, the Live Activity and the control), `ios/App/Shared` (compiled into both: the activity's types, the rest controls, the snapshot), `ios/App/Watch` (the watch app), `ios/App/WatchWidgets` and `ios/App/WatchShared` (its complications). `scripts/ios-configure-project.rb` edits the Xcode project without Xcode.
...
- **Build numbers** count up by themselves (`run number.attempt`). **To ship a new version number**, bump `MARKETING_VERSION` on every target in `ios/App/App.xcodeproj` (the app, the widgets and the watch must match). The app is on **1.5**.
