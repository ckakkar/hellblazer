 ...
 * Authenticated by the phone's device token (a watch's is refused); unit and
 * timezone come in the same X-Fatty-* headers.
 */
export async function GET(request: Request) {
  return handleWatch(
    request,
    async (ctx) => ({ ...(await buildWidgetSnapshot(ctx.db, ctx)), updatedAt: Date.now() }),
    // The phone's token only: it reads, where a watch's can write.
    "phone",
  );
}
