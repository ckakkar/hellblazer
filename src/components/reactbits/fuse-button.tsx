...
/*
 * FuseButton, vendored from React Bits (reactbits.dev, FuseButton-TS-TW) via
 * the shadcn MCP registry. Local changes: lucide icons instead of Hugeicons
 * (no new icon dependency), a 48px large size to match the app's buttons,
 * and the app's colours, with the fuse burning in the lifter's accent. The
 * fuse is a Web Animation, so it runs on the compositor.
 */
...
const SIZES: Record<FuseSize, { height: number; font: number; icon: number; px: number }> = {
  ...
  lg: { height: 48, font: 15, icon: 17, px: 20 }
};
