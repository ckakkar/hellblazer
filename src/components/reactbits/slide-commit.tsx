...
/*
 * SlideCommit, vendored from React Bits (reactbits.dev, SlideCommit-TS-TW) via
 * the shadcn MCP registry. Local changes: lucide icons instead of Hugeicons,
 * fills its container when no width is given (measured with a
 * ResizeObserver), the app's colours with the success fill in the lifter's
 * accent (a CSS colour, so its text colour is passed explicitly), and no
 * bounce or landing dip: it eases home and stops.
 */
...
export interface SlideCommitProps {
  ...
  successColor?: string;
  dangerColor?: string;
  /** Text/icon colour on the success fill; worked out from the fill when it's a hex. */
...
const SlideCommit: React.FC<SlideCommitProps> = ({
  ...
  trackColor = '#1c1c1f',
  handleColor = '#f4f2ee',
  successColor = 'rgb(var(--accent-rgb))',
  dangerColor = '#ef5350',
  successTextColor,
  width: fixedWidth,
  height = 56,
  ...
  // Fill the container unless a width is given: measure it, and keep the
  // geometry below in step when the layout changes.
  const rootRef = useRef<HTMLDivElement>(null);
  const [measured, setMeasured] = useState(fixedWidth ?? 280);
  useLayoutEffect(() => {
    if (fixedWidth !== undefined || !rootRef.current) return;
    const el = rootRef.current;
    const read = () => setMeasured(Math.max(160, Math.round(el.clientWidth)));
    read();
      ...
      style={
        {
          ...
          '--sc-on-ok': successTextColor ?? onColor(successColor),
        ...
        >
          <motion.div className="absolute inset-0" style={{ transform: content }}>
            <motion.span
              ...
              aria-hidden="true"
            >
              {icon ?? <ChevronsRight size={iconSize} strokeWidth={2.25} />}
            </motion.span>
            <motion.span
