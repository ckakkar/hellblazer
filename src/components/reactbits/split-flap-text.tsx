...
/**
 * React Bits `SplitFlapText`: a mechanical departure board that clacks through
 * to each new phrase. See ./README.md for provenance and local changes.
 *
 * Local changes beyond the usual re-theming:
 *
 * - **Fixed the `gap` prop.** Upstream publishes `--split-flap-gap` but never
 *   references it in the stylesheet, so the prop did nothing and the tiles sat
 *   flush. The rule below applies it.
 * - **`role="text"` removed.** That role isn't in the ARIA spec (Safari-only),
 *   and the tiles are already `aria-hidden`, so the board announced nothing.
 *   The settled phrase is now exposed through a visually-hidden live region.
 * - Defaults re-pointed at the app's tokens, so the board reads as the same
 *   machined material as the rest of the instrument panel.
 *
 * On cost: the rAF here is not a standing loop. It runs only while tiles are
 * mid-flip and cancels itself when they settle, with a `setTimeout` between
 * phrases; a single-phrase board never starts one at all.
 */
...
const styles = `
.split-flap-text{font-family:var(--font-mono),ui-monospace,Menlo,monospace;font-size:var(--split-flap-font-size,52px);font-weight:760;line-height:1;letter-spacing:.035em;font-variant-numeric:tabular-nums;gap:var(--split-flap-gap,6px)}
...
const SplitFlapText = ({
  ...
  flipsPerChar = 8,
  // App tokens: the tile is the elevated surface, the glyph is body text.
  ...
  // Upstream reset the board inside the effect below, which is a setState in an
  // effect body: it renders once with stale tiles, then immediately again.
  // React's documented way to react to changed props is to adjust state during
  // render, tracking the previous value in state (not a ref, which must not be
  // read while rendering).
  const firstPhrase = normalizedPhrases[0] || '';
  const [seededPhrase, setSeededPhrase] = useState(firstPhrase);
  if (seededPhrase !== firstPhrase) {
    setSeededPhrase(firstPhrase);
  ...
  useEffect(() => {
    ...
    const scheduleNext = (delay: number) => {
      // Plain setTimeout, not window.setTimeout: the ref is typed from the
      // global setTimeout, whose return type differs under @types/node.
  ...
  return (
    <>
      <style>{styles}</style>
      {/* The tiles are decorative; this is what a screen reader actually gets. */}
